...
| `download_only`             | bool     | Optional  | Only download the update. Skip the uninstall and install steps.   |
...
When set to `true`, the module will only download the update and then stop, skipping the uninstall and install steps. The downloaded binary is left in place (in `download_destination`, or the default `temp` cache directory) rather than being removed after use. The `DoCommand` response reports the path of the downloaded file, for example:

```json
{
  "downloaded": "C:\\path\\to\\downloaded\\setup.exe"
}
```

A later `DoCommand` with `download_only` unset (or `false`) will reuse this cached download instead of fetching it again, as long as the URL and file are unchanged.
...
For example, to download the update now but defer installation to a later call:
